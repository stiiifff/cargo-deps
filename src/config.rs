...
impl<'a> Config<'a> {
    ...
    pub fn from_matches(m: &'a ArgMatches) -> CliResult<Self> {
        Ok(Config {
            ...
            dev_deps: m.value_of("dev-deps").unwrap_or("false").parse_arg()?,
            build_deps: m.value_of("build-deps").unwrap_or("true").parse_arg()?,
            optional_deps: m.value_of("optional-deps").unwrap_or("true").parse_arg()?,
